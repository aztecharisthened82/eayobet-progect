...
class MoneyOutController extends GetxController {
  ...
  RxDouble requestAmount = 0.00.obs;
  RxDouble totalCharge = 0.00.obs;
  RxDouble youWillGet = 0.00.obs;
  RxDouble exchangeRate = 0.00.obs;
  ...
  Future<DynamicInputWithdrawModel> manualPaymentGetGatewaysProcess() async {
    ...
    await ApiServices.withdrawInsertApi(body: inputBody)
        .then((value) {
          _moneyOutInsertModel = value!;

          requestAmount.value =
              double.tryParse(
                _moneyOutInsertModel.data.paymentInformations.requestAmount,
              ) ??
              0.0;
          totalCharge.value =
              double.tryParse(
                _moneyOutInsertModel.data.paymentInformations.totalCharge,
              ) ??
              0.0;
          youWillGet.value =
              double.tryParse(
                _moneyOutInsertModel.data.paymentInformations.willGet,
              ) ??
              0.0;
          exchangeRate.value =
              double.tryParse(
                _moneyOutInsertModel.data.paymentInformations.exchangeRate,
              ) ??
              0.0;
          payableAmount.value =
              double.tryParse(
                _moneyOutInsertModel.data.paymentInformations.payable,
              ) ??
              0.0;
